...
- **CS-STAN 402c / Part ML**: governs installation of onboard recorders and maintenance requirements.
...
The generic value $b=-0.16$ is therefore chosen because it is slightly steeper than the steepest retained GFRP epoxy value and remains strongly conservative for CFRP-dominated epoxy structures. The selected value shall be treated as a conservative EFI severity-weighting parameter for the generic SLM methodology. It is not used to establish an absolute fatigue life, to substantiate a specific structural detail, or to replace the original Type Certification structural substantiation.
...
This appendix defines the processing rules used to convert a measured load-factor time history into a load-factor occurrence spectrum using the Kossira & Reinke counting method.

The objective of this appendix is not to introduce a new counting method, but to make explicit the implementation conventions required to reproduce the Kossira & Reinke method consistently in software.

#### A.1 Input signal and class definition

The input signal is the vertical load factor, noted (N_z), during valid aerodynamic flight phases.

The load-factor range is divided over the interval:

$$-4g \leq N_z \leq +6g$$

The signal is first represented at $$2^{10}$$ resolution, corresponding to 1024 fine classes over this range. The fine-class resolution is therefore approximately 0.01g.

A reduced representation using $$2^5$$ resolution is then used for counting, corresponding to 32 load-factor classes over the same range. The 32-class width is approximately 0.32g.

The class-number convention used in this document is:

```text
class 1  = lowest load-factor class
class 32 = highest load-factor class
```

#### A.2 Hysteresis filtering in 1024-class representation

The Kossira & Reinke hysteresis filter is applied before reduction to 32 classes.

A new sample is retained only if the absolute difference between its 1024-class value and the last retained 1024-class value is greater than the threshold $$DX$$. Kossira & Reinke used:

$$DX = 10$$

in the 1024-class representation, corresponding to approximately 0.1g.

This filtering prevents small oscillations, especially near 32-class boundaries, from producing artificial peak/valley transitions.

The retained 1024-class sequence is then reduced to the corresponding 32-class sequence.

#### A.3 Peak and valley extraction

The filtered 32-class sequence is reduced to alternating local maxima and minima, also called peaks and valleys.

Successive values on the same monotonic branch are not counted as separate transitions. Only the extremum of the branch is retained. The resulting sequence is therefore an alternating sequence of valleys and peaks:

```text
valley → peak → valley → peak → ...
```

This sequence is the input to the Markov transition matrix.

#### A.4 Initial Markov transition matrix

A 32 × 32 Markov transition matrix is filled using consecutive peak/valley values.
...
the corresponding matrix cell is incremented by one.

The initial Markov matrix only counts direct peak/valley endpoint transitions. It does not yet count all intermediate classes passed through by the load variation.
...
in the initial Markov matrix.

It does not directly increment classes 17, 18, 19, or 20. Those pass-through/exceedance counts are produced by the matrix transformation described below.

#### A.5 Mean value used for Markov-matrix splitting

The mean value (MV) used to split the Markov matrix is calculated from the retained 32-class sequence used for peak/valley counting.

It is therefore expressed in continuous 32-class coordinates, not as a raw load-factor value and not as a re-binned integer class.
...
means that the mean value lies between class 16 and class 17 in the reduced 32-class coordinate system.

The split is then defined as:

```text
last negative-side class  = floor(MV)
first positive-side class = floor(MV) + 1
```
...
negative side: classes 1 to 16
positive side: classes 17 to 32
```

This convention treats (MV) as a boundary between the negative and positive Markov transformations. The mean value itself is not inserted as an additional class and is not used as a row in the matrix.

If (MV) is exactly an integer value, the integer class belongs to the negative side and the next class begins the positive side:
...
This rule avoids ambiguity and shall be applied consistently.

#### A.6 Transformation to cumulative exceedance matrix

The initial Markov matrix is transformed into a cumulative exceedance matrix so that each cell represents the number of times a load variation reached or passed through the corresponding class level.

The transformation is performed separately on the positive and negative sides of the matrix.

##### Positive side

The positive side includes rows:

```text
first positive-side class to class 32
```

Only cells above the diagonal are used, corresponding to upward valley-to-peak transitions:

```text
to class > from class
```

For each positive-side row and column, the transformed cell is obtained by adding the current Markov cell and all Markov cells above it in the same column, i.e. all higher `to` classes in the same `from` column.

In class-number notation, for a positive-side row (r) and column (c):

```text
use only r > c
transformed[r,c] = sum of Markov[q,c] for q = r to 32
```

subject to the positive-side row limits.

##### Negative side

The negative side includes rows:

```text
class 1 to last negative-side class
```

Only cells below the diagonal are used, corresponding to downward peak-to-valley transitions:

```text
to class < from class
```

For each negative-side row and column, the transformed cell is obtained by adding the current Markov cell and all Markov cells below it in the same column, i.e. all lower `to` classes in the same `from` column.

In class-number notation, for a negative-side row (r) and column (c):

```text
use only r < c
transformed[r,c] = sum of Markov[q,c] for q = 1 to r
```

subject to the negative-side row limits.

##### Diagonal and unused cells

The diagonal does not represent a load variation and is set to zero in the transformed matrix.

Cells on the wrong side of the mean-value split are not used and are set to zero.

The transformed matrix is therefore not another direct transition matrix. It is a cumulative exceedance matrix derived from the initial peak/valley Markov transition matrix.

#### A.7 Occurrence spectrum

The load-factor occurrence spectrum is obtained by summing the rows of the transformed matrix.

For positive-side rows, only above-diagonal transformed cells are summed.

For negative-side rows, only below-diagonal transformed cells are summed.

The result is the number of occurrences associated with each 32-class load-factor level.

This occurrence spectrum is the spectrum used in the EFI calculation.

#### A.8 Consistency rule

The following items shall be controlled in the implementation:

- load-factor range;
- 1024-class definition;
- hysteresis threshold (DX);
- conversion from 1024 classes to 32 classes;
- peak/valley extraction rule;
- Markov matrix convention;
- mean-value calculation;
- Markov-matrix split rule;
- cumulative matrix transformation;
- occurrence row-sum rule.

This consistency is essential because different counting methods or different implementation conventions can produce different occurrence distributions.

The SLM method compares the real spectrum to the reference spectrum by processing the real measured spectrum with the same method used for the reference spectrum, i.e. the Kossira & Reinke method.

#### A.9 Reference occurrence table

Implementation of the method requires the applicable Kossira & Reinke reference occurrence table in machine-readable form.

The table used for certification or airworthiness decisions shall be controlled as part of the approved calculation package.
