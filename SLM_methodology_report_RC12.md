...
**Revision Candidate 11 (RC11) — updated reference-source proposal**
...
For the approved SLM calculation package, the Utility reference occurrence table shall be generated from the published ABB.163 Utility Markov matrix using the controlled Markov-matrix-to-occurrence-spectrum transformation defined by Kossira & Reinke and described in Appendix A.8 and verified in Appendix C. 
...
2. Periods or phases without valid SLM data, including non-flight phases, are not included in the measured aerodynamic spectrum. Any separate operational accounting for these periods is controlled by the approved ICA.
...
For real measured data, the same class mapping, filtering rule, peak/valley rule, Markov-matrix convention, average-load-factor convention, and exceedance transformation shall be used whenever raw samples are processed into an occurrence spectrum. The Kossira & Reinke Utility reference spectrum used for EFI shall enter the operational calculation directly as an approved machine-readable occurrence table generated from the published ABB.163 Utility Markov matrix using the same controlled transformation. The EFI calculation shall apply the same approved Kossira & Reinke load-factor class table and EFI equations to the ABB.163-derived reference occurrence table and to the real measured occurrence table generated by the software. No interpolation onto a different load-factor grid shall be introduced between spectrum generation and EFI calculation. The detailed software requirements for the Kossira & Reinke processing chain are defined in Appendix A. The detailed software requirements for EFI calculation and reporting are defined in Appendix B. The ABB.163-to-ABB.164 verification supporting the adopted Kossira & Reinke class convention, the Markov-matrix transformation, and the ABB.163-derived reference occurrence table is documented in Appendix C.
...
The selected conversion rule shall be applied consistently to real-flight processing, verification data, and any inverse synthetic-data generation used for tool validation. For the Kossira & Reinke Utility reference case, the approved occurrence table shall be generated from the published ABB.163 Markov matrix using the Appendix A.8 transformation and the same approved class definitions used by this appendix. Once generated and verified, this occurrence table may be stored and imported directly by EFI software as a controlled reference-data item.
...
1. the approved Utility reference occurrence table generated from the published ABB.163 Utility Markov matrix;
2. the approved Kossira & Reinke load-factor class table;
...
The published Utility Markov matrix of ABB.163 shall be treated as the primary numerical source for generating the approved Utility reference occurrence table.
...
Implementation of the method requires the applicable Kossira & Reinke Utility reference occurrence table. That table shall be generated from the published ABB.163 Utility Markov matrix using the controlled transformation of Appendix A.8 and shall be included in the approved calculation package in machine-readable form.

The approved reference occurrence table generated from ABB.163 is the operational reference input for EFI. The approved load-factor class table shall use the Kossira & Reinke class convention validated in Appendix C. No interpolation to another load-factor grid shall be performed between the occurrence spectrum and the EFI calculation.
...
1. approved Kossira & Reinke Utility reference occurrence table generated from the ABB.163 Utility Markov matrix and normalized to the approved reference duration;
2. valid real aerodynamic recording duration associated with the real measured occurrence table;
3. real measured occurrence table generated by Appendix A;
...
### 10. Appendix C — Generation and Verification of the ABB.163-Derived Utility Reference Spectrum

This appendix documents the generation of the numerical Kossira & Reinke Utility reference occurrence spectrum from the published ABB.163 Markov matrix and the verification showing that the generated spectrum reproduces the published ABB.164 Utility occurrence curve.

#### C.1 Purpose and verification scope

The reference-data generation and verification are limited to the following part of the method:

```text
32×32 Kossira & Reinke Markov transition matrix
→ transformed exceedance matrix
→ load-factor occurrence spectrum
```

This verification is important for two reasons. First, the SLM methodology uses the same Markov-matrix-to-occurrence-spectrum transformation to generate measured real-flight spectra from recorded load-factor data. Second, ABB.163 provides a more precise numerical source for the Utility reference spectrum than digitizing the plotted ABB.164 curve.

#### C.2 Source material

The verification uses the following original Kossira & Reinke published material from the Utility reference case:

1. **ABB.163** — published 32×32 Markov transition matrix;
2. **ABB.164** — published load-factor occurrence curves resulting from the Kossira & Reinke processing.

ABB.163 is used as the numerical source for generating the Utility reference occurrence spectrum. ABB.164 is used as the graphical validation reference for the resulting occurrence spectrum. The operational EFI reference input is the approved machine-readable occurrence table generated from ABB.163, not a digitized numerical extraction from ABB.164.

#### C.3 Kossira & Reinke class convention verified from ABB.163 and ABB.164

The ABB.163-to-ABB.164 comparison showed that the Utility reference data and the generated reference occurrence table are consistent with the following coarse class spacing:

$$
\Delta LF_{coarse}=0.375g
$$

The corresponding fine-class spacing is:

$$
\Delta LF_{fine}=\frac{0.375}{32}=0.01171875g
$$

The Kossira & Reinke threshold is therefore:

$$
DX=10\Delta LF_{fine}=0.1171875g
$$

For the Utility reference data, the internal low-to-high load-factor class convention used by the SLM software is:
...
This class table is used directly for both the ABB.163-derived reference occurrence spectrum and the measured real-flight occurrence spectrum. No interpolation onto a different load-factor grid is introduced for EFI calculation.

#### C.4 ABB.163 matrix orientation and import convention

The ABB.163 table is printed using its original Kossira & Reinke layout. For software processing, the printed table shall be imported into the internal low-to-high load-factor convention used by Appendix A.

The approved calculation package shall therefore include the machine-readable ABB.163 import table and the associated mapping between:

```text
printed ABB.163 row/column labels
```

and:

```text
internal SLM low-to-high load-factor class index
```

This mapping is part of the ABB.163 reference-data import and reference-spectrum generation procedure. It is not part of the general Kossira & Reinke algorithm applied to real measured data.

#### C.5 Transformation rule verified
...
- $j$ is the “from” class;
- $i$ is the “to” class;
- $M(i,j)$ is the number of transitions from class $j$ to class $i$.

The transformed occurrence spectrum is computed by splitting the matrix around the average-load-factor region.
...
The split class used for the ABB.163 Utility reference-spectrum generation is the split that aligns the lower and upper branches around the approximately 1g average-load-factor region and reproduces the ABB.164 Utility curve within graph-digitization accuracy.

#### C.6 Verification result

The ABB.163 Markov matrix was digitized into a 32×32 machine-readable matrix and processed using the transformation defined in Appendix A.8. The resulting computed occurrence spectrum is retained as the numerical Utility reference occurrence table for EFI calculation.

The generated occurrence spectrum was then compared with the ABB.164 Utility occurrence curve digitized from the published graph as an independent graphical validation check.

The ABB.163-derived reference spectrum and the digitized ABB.164 curve agreed within the expected accuracy of graph digitization.
...
The close agreement between the ABB.163-derived occurrence spectrum and the ABB.164 published Utility occurrence curve confirms that the Markov-matrix-to-occurrence-spectrum transformation used by the SLM methodology is consistent with the original Kossira & Reinke method, within the accuracy limits of graph digitization. It also justifies using the ABB.163-derived occurrence table as the operational numerical Utility reference spectrum, rather than using a graph-digitized ABB.164 table.

This verification also supports the use of the original Kossira & Reinke class hypothesis, with 32 coarse classes spaced by approximately 0.375g.

#### C.8 Conclusion

This verification confirms the Markov-matrix-to-occurrence-spectrum transformation, the associated Kossira & Reinke class convention, and the use of the ABB.163-derived occurrence table as the numerical Utility reference spectrum.
