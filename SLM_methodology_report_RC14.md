...
**Revision Candidate 13 (RC13) — simplified ABB.163 reference-spectrum proposal**
...
The reference load spectrum for this methodology is the Kossira & Reinke Utility spectrum [2] [8]. This is a deliberate initial-scope limitation: SLM is proposed at this stage only for gliders operated within Utility-category limitations. Aerobatic-category operation and any operation outside the Utility envelope are not credited under this methodology. The Utility spectrum is recognized by EASA (CM-S-006) as a basis for sailplane fatigue analysis.

For the approved SLM calculation package, the numerical Utility reference occurrence table shall be generated from the published ABB.163 Utility Markov matrix using the controlled Markov-matrix-to-occurrence-spectrum transformation of Appendix A.8. This ABB.163-derived occurrence table is the operational reference input for EFI.

ABB.164 is not used as a digitized numerical reference table. It is used as the original published graphical check showing that the ABB.163-derived occurrence spectrum reproduces the Kossira & Reinke Utility curve within graph-digitization accuracy. This verification is documented in Appendix C.

The reference occurrence table is normalized to the reference duration defined in the approved calculation package. In the generic SLM equations, the working reference normalization is 6,000 flight hours. Because the certification EFI limit is anchored at the 12,000-hour life (§3), the reference occurrences are scaled to the required exposure time before use in EFI calculations. The original Kossira & Reinke spectrum is used rather than the more recent KoSMOS variant: KoSMOS omits low load factor variations to shorten ground-test duration, but those omissions are not appropriate when the reference spectrum is being compared cycle-for-cycle against a real measured spectrum.

#### 4.1.1 Use of the reference spectrum in the methodology

The ABB.163-derived Utility reference spectrum is used whenever EFI must be assessed against the certified reference basis:

1. For EFI calibration, the reference spectrum is scaled to the certified 12,000-hour life to anchor the EFI model to the certification EFI limit (§4.4).
2. For life-extension projection, the reference EFI is compared with the EFI produced by real flights over the same valid real-flight duration (§4.5).
3. Periods or phases without valid SLM data are not included in the measured aerodynamic spectrum. Any separate operational accounting for such periods is controlled by the approved ICA, IOMM, or operational procedure.
...
For real measured data, the same class mapping, filtering rule, peak/valley rule, Markov-matrix convention, average-load-factor convention, and exceedance transformation shall be used whenever raw samples are processed into an occurrence spectrum. The reference side of the calculation uses the approved ABB.163-derived occurrence table generated with the same Markov-matrix-to-spectrum transformation. Both spectra therefore use the same Kossira & Reinke class table and no interpolation is introduced before EFI calculation.

Appendix A defines the Kossira & Reinke processing requirements, Appendix B defines the EFI calculation rules, and Appendix C documents the ABB.163-to-ABB.164 verification that supports the selected class convention and reference spectrum.
...
Operation beyond the certified life limit is a continuation of Phase 1 monitoring with additional continued-airworthiness controls. A Go/No-Go decision is made at each annual visit, ARC renewal or other approved review point using the validated SLM EFI status report. The Go criterion shall include both the current cumulative EFI and the projected EFI rate: $EFI_{total}$ must remain below $1/3$, and the projected EFI rate must show that the $EFI=1/3$ limit will not be reached before the next scheduled inspection or review. Recurring structural inspections continue at a maximum interval of 1,000 flight hours or 5 years, whichever occurs first. The process continues until the aircraft exhausts its remaining EFI potential or another approved continued-airworthiness limit is reached.
...
The Kossira & Reinke processing chain uses two related digital resolutions:
...
The SLM methodology shall use the Kossira & Reinke class convention validated against the Utility reference material.

For the Utility reference calculation, the approved Kossira & Reinke coarse class spacing shall be:

$$
\Delta LF_{coarse}=0.375g
$$

The corresponding fine-class spacing shall be:

$$
\Delta LF_{fine}=\frac{\Delta LF_{coarse}}{32}=0.01171875g
$$
...
$$
DX_{g}=10\Delta LF_{fine}=0.1171875g
$$

The approved Utility class table shall be included in the calculation package in machine-readable form. For the Utility reference case validated in Appendix C, the practical internal class convention is:

$$
LF(c)=LF_0+c \ \Delta LF_{coarse}
$$
...
Equivalently, using one-based notation:
...
The class values defined above are the Kossira & Reinke load-factor levels used for spectrum generation and EFI calculation. The purpose of using the Kossira & Reinke class table directly is to compare reference and real spectra on the same load-factor grid and avoid interpolation.

The implementation shall use zero-based class numbering internally unless otherwise specified:
...
For each valid load-factor sample $LF$, the corresponding 1024-class value shall be computed using the approved Kossira & Reinke fine-class table. For the Utility class convention of Appendix A.1, the default conversion is:

$$
q_{1024}=\mathrm{clip}\left(
\left\lfloor
\frac{LF-LF_0}{\Delta LF_{fine}}
\right\rfloor,
0,
1023
\right)
...
The load-factor level associated with a fine class is:

$$
LF(q_{1024}) = LF_0 + q_{1024}\Delta LF_{fine}
$$

Samples clipped to the first or last fine class shall be retained according to the normal processing rules, but clipping events shall be counted and reported as processing warnings when detailed data-quality reporting is required.

The selected conversion rule shall be applied consistently to real-flight processing, verification data, and any inverse synthetic-data generation used for tool validation. The approved Utility reference table is generated once from ABB.163 using Appendix A.8, verified against ABB.164, and then controlled as a reference-data item for EFI software.
...
The load-factor level associated with a coarse class is:

$$
LF(q_{32}) = LF_0 + q_{32}\Delta LF_{coarse}
$$
...
The corresponding average 32-class index used to split the Markov matrix shall be computed using the approved Kossira & Reinke class table. For the Utility class convention of Appendix A.1, the default index is:

$$
k_{avg,real}=\mathrm{clip}\left(
\left\lfloor
\frac{LF_{average,real}-LF_0}{\Delta LF_{coarse}}
\right\rfloor,
0,
31
\right)
...
The approved calculation package shall provide the reference data needed by the EFI software:

1. the ABB.163-derived Utility occurrence table;
2. the Kossira & Reinke load-factor class table;
...
When a raw 32×32 Markov matrix is generated from real measured data, it shall be transformed into a 32-class occurrence spectrum by counting upward and downward exceedances relative to the applicable average class $k_{avg}$.
...
With this default convention, the average class belongs to the upper branch. No additional zeroing, smoothing, or replacement of the average-class occurrence shall be performed.
...
Implementation of the method requires the approved Kossira & Reinke Utility reference occurrence table and the associated load-factor class table. The reference occurrence table shall be generated from ABB.163 using Appendix A.8, verified as described in Appendix C, and included in the approved calculation package in machine-readable form.

No interpolation to another load-factor grid shall be performed between the occurrence spectrum and the EFI calculation.
...
1. approved ABB.163-derived Kossira & Reinke Utility reference occurrence table;
...
4. approved Kossira & Reinke load-factor class table;
...
The EFI calculation shall not compute a second independent average from the occurrence table. This avoids mixing a time-history or Markov-splitting average with an occurrence-weighted average, which could produce different numerical values.

#### B.3 Reference spectrum scaling

The approved Kossira & Reinke Utility occurrence table is generated from the ABB.163 Utility Markov matrix and normalized to the approved reference duration defined in the reference-data package. For the generic SLM methodology, the working reference normalization used in the EFI equations is 6,000 flight hours.
...
This appendix documents the generation of the numerical Kossira & Reinke Utility reference occurrence spectrum from the published ABB.163 Markov matrix and the verification against the published ABB.164 Utility curve.

#### C.1 Purpose

The purpose of this appendix is to close the reference-data chain used by the SLM calculation:

```text
ABB.163 Utility Markov matrix
→ Kossira & Reinke Markov-matrix-to-spectrum transformation
→ ABB.163-derived Utility occurrence spectrum
→ EFI reference input
```

ABB.164 is used only as a graphical validation of this generated spectrum. It is not used as the operational numerical reference table because the published document provides a curve rather than tabulated values.

#### C.2 Source material

The verification uses the following Kossira & Reinke material:

1. **ABB.163** — published 32×32 Utility Markov transition matrix;
2. **ABB.164** — published Utility occurrence curve.

ABB.163 is the primary numerical source for the Utility reference occurrence spectrum. ABB.164 is the independent graphical check that the transformation and class convention reproduce the published Kossira & Reinke result.

#### C.3 Class convention

The ABB.163-to-ABB.164 comparison supports the following Kossira & Reinke class convention for the Utility reference data:
...
$$
\Delta LF_{fine}=\frac{0.375}{32}=0.01171875g
$$

$$
DX=10\Delta LF_{fine}=0.1171875g
$$

The internal low-to-high load-factor class convention is:
...
The same class table shall be used for the ABB.163-derived reference spectrum and for real measured spectra. No interpolation onto a different load-factor grid shall be introduced for EFI calculation.

#### C.4 ABB.163 import convention

ABB.163 is printed using its original Kossira & Reinke table layout. For software processing, the printed matrix is imported into the internal low-to-high load-factor convention used by Appendix A.

For the digitized ABB.163 Utility matrix, the relationship between the printed table label $L$ and the internal low-to-high class index $q$ is:

$$
q=33-L
$$

The approved calculation package shall include the machine-readable ABB.163 matrix and this import mapping. This mapping is specific to ABB.163 reference-data generation. It is not part of the general algorithm applied to real measured data.

#### C.5 Transformation rule

The raw Markov transition matrix is defined as:

$$
M(i,j)
$$

where $j$ is the “from” class, $i$ is the “to” class, and $M(i,j)$ is the number of transitions from class $j$ to class $i$.

For the lower branch, the occurrence at class $k$ counts transitions from above class $k$ to class $k$ or below:

$$
O(k)=\sum_{i=0}^{k}\sum_{j=k+1}^{31}M(i,j)
$$

For the upper branch, the occurrence at class $k$ counts transitions from below class $k$ to class $k$ or above:

$$
O(k)=\sum_{i=k}^{31}\sum_{j=0}^{k-1}M(i,j)
$$

For the Utility reference generation, the split is selected around the approximately 1g average-load-factor region. The same rule is used for real measured spectra with the real-data average class defined by Appendix A.7.

#### C.6 Generated ABB.163 Utility reference spectrum

The following table is the ABB.163-derived Utility occurrence spectrum generated with the Appendix A.8 transformation and the Kossira & Reinke class convention of Appendix C.3. This table, in controlled machine-readable form, is the operational numerical reference spectrum for EFI.

| q | ABB.163 printed label | LF (g) | Occurrences |
|---:|---:|---:|---:|
| 1 | 32 | -4.000 | 0 |
| 2 | 31 | -3.625 | 0 |
| 3 | 30 | -3.250 | 0 |
...
| 9 | 24 | -1.000 | 15,111 |
| 10 | 23 | -0.625 | 258,197 |
| 11 | 22 | -0.250 | 1,229,659 |
| 12 | 21 | 0.125 | 776,762 |
| 13 | 20 | 0.500 | 880,781 |
| 14 | 19 | 0.875 | 7,638,649 |
| 15 | 18 | 1.250 | 7,615,484 |
| 16 | 17 | 1.625 | 5,018,617 |
| 17 | 16 | 2.000 | 781,639 |
| 18 | 15 | 2.375 | 222,911 |
| 19 | 14 | 2.750 | 115,715 |
| 20 | 13 | 3.125 | 64,698 |
...
#### C.7 Validation against ABB.164

The generated ABB.163 spectrum was compared with the ABB.164 Utility occurrence curve digitized from the published graph. Two digitization corrections were applied before comparison:

1. one ambiguous central point near the peak was removed because it was inconsistent with the class sequence and likely came from overlapping plotted markers;
2. one high-load point near approximately 6.65g was corrected to the intended nearby marker.

After these corrections, the ABB.163-derived spectrum and ABB.164 curve agreed within the expected accuracy of graph digitization:

```text
Compared points:                      27
Median absolute relative difference:   approximately 2.7 %
Mean absolute relative difference:     approximately 2.9 %
Maximum absolute relative difference:  approximately 10 %
Points within ±5 %:                   25 / 27
Points within ±10 %:                  27 / 27
```

The maximum relative difference occurs at a very low occurrence count, where the absolute difference is one occurrence. Excluding this small-count point, the maximum relative difference is approximately 6%.

#### C.8 Conclusion

The agreement between the ABB.163-derived occurrence spectrum and the ABB.164 published Utility curve confirms, within graph-digitization accuracy, the Markov-matrix-to-occurrence-spectrum transformation and the Kossira & Reinke class convention used by this methodology.

Consequently, the SLM reference spectrum shall be the controlled ABB.163-derived occurrence table, not a digitized extraction of ABB.164. ABB.164 remains the graphical validation evidence.
