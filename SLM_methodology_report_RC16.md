...
**Revision Candidate 15 (RC15) — simplified ABB.163 reference-spectrum proposal**
...
For the Utility reference case, the approved Kossira & Reinke class convention is:
...
$$
DX_g=10\Delta LF_{fine}=0.1171875g
$$

The approved Utility class table shall be included in the calculation package. In zero-based notation:
...
where $LF_0=-4.0g$ and $\mathrm{clip}(x,0,1023)$ limits the value to the valid range.

The load-factor value associated with a fine class is:
...
The corresponding average 32-class index is:
...
The approved reference-data package shall provide $LF_{average,ref}$ for the ABB.163-derived Utility reference spectrum.
...
#### C.2 Class convention

The ABB.163-derived reference spectrum and the real measured spectra use the same Kossira & Reinke class convention:
...
and the Kossira & Reinke hysteresis threshold is:
...
The following table is the ABB.163-derived Utility occurrence spectrum generated with the Appendix A.8 transformation. In controlled machine-readable form, it is the operational numerical reference spectrum for EFI.
...
#### C.4 Validation against ABB.164

The generated ABB.163 spectrum was compared with the ABB.164 Utility occurrence curve digitized from the published graph.
